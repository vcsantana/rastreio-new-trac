...
class CommandTemplateService:
    ...
    async def update_template(self, template_id: int, update_data: CommandTemplateUpdate, user_id: int) -> Optional[CommandTemplate]:
        ...
        try:
            template = self.db.query(CommandTemplate).filter(
                and_(
                    CommandTemplate.id == template_id,
                    CommandTemplate.user_id == user_id,
                    CommandTemplate.is_active == True
            ...
            if update_data.name is not None:
                ...
                existing_template = self.db.query(CommandTemplate).filter(
                    and_(
    ...
    async def get_template_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            base_query = self.db.query(CommandTemplate).filter(
                or_(
                    CommandTemplate.user_id == user_id,
                    CommandTemplate.is_public == True
                )
            ...
            # Public vs private
            public_templates = base_query.filter(CommandTemplate.is_public == True).count()
            ...
            # Active vs inactive
            active_templates = base_query.filter(CommandTemplate.is_active == True).count()
            inactive_templates = base_query.filter(CommandTemplate.is_active == False).count()
            
            # By command type
            command_type_stats = {}
            for cmd_type in CommandType:
                count = base_query.filter(CommandTemplate.command_type == cmd_type.value).count()
                if count > 0:
                    command_type_stats[cmd_type.value] = count
            
            # Most used templates
            most_used = base_query.filter(CommandTemplate.usage_count > 0).order_by(
                desc(CommandTemplate.usage_count)
            ).limit(5).all()
            ...
            # Recent templates
            recent_templates = base_query.order_by(desc(CommandTemplate.created_at)).limit(5).all()
            ...
            templates_created_last_week = base_query.filter(
                CommandTemplate.created_at >= week_ago
            ).count()
            
            templates_used_last_week = base_query.filter(
                CommandTemplate.last_used_at >= week_ago
            ).count()
    ...
    async def create_scheduled_command(self, scheduled_data: ScheduledCommandCreate, user_id: int) -> ScheduledCommand:
        ...
        try:
            ...
            existing_scheduled = self.db.query(ScheduledCommand).filter(
            ...
            self.db.commit()
            self.db.refresh(scheduled_command)
    ...
    async def get_scheduled_commands(self, page: int, size: int, is_active: Optional[bool], 
        ...
        try:
            query = self.db.query(ScheduledCommand).filter(
                ScheduledCommand.user_id == user_id
            )
            
            # Apply filters
            if is_active is not None:
                query = query.filter(ScheduledCommand.is_active == is_active)
            
            if is_executed is not None:
                query = query.filter(ScheduledCommand.is_executed == is_executed)
            ...
            # Apply pagination
            offset = (page - 1) * size
            scheduled_commands = query.order_by(desc(ScheduledCommand.scheduled_at)).offset(offset).limit(size).all()
            
            return scheduled_commands, total
    ...
    async def get_scheduled_command(self, scheduled_id: int, user_id: int) -> Optional[ScheduledCommand]:
        ...
        try:
            scheduled_command = self.db.query(ScheduledCommand).filter(
                and_(
    ...
    async def delete_scheduled_command(self, scheduled_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            self.db.delete(scheduled_command)
            self.db.commit()
